...
class Configuration:
    ...
    def __init__(self):
        ...
        self.aws_rx_prefix = 'airglow/raw/'
        self.aws_data_prefix = 'airglow/'
        self.aws_results_prefix = 'airglow/fpi/results/'
        
        # Local temporary directory
        self.temp_dir = os.getenv('TEMP_DIR', '/tmp/mango')
        ...
        self.fpi_process_kwargs = {
            ...
            'bw_dir': f"{self.temp_dir}/templogs/cloudsensor/",
...
class FPIHandler(InstrumentHandler):
    ...
    def process_data(self, metadata: FileMetadata, sorted_files: List[str]):
        ...
        try:
            ...
            if results_dir.exists():
                cloud_results_prefix = f"{self.config.aws_results_prefix}fpi/minime{metadata.instrument_number}/{metadata.site}/{year}/"
    ...
    def _upload_directory_to_cloud(self, local_dir: Path, cloud_prefix: str):
        """Upload a directory and its contents to cloud storage."""
...
class CloudSensorHandler(InstrumentHandler):
    ...
    def get_cloud_target_prefix(self, metadata: FileMetadata) -> str:
        ...
        return f"{self.config.aws_data_prefix}templogs/cloudsensor/{metadata.site}/"

    def get_local_target_directory(self, metadata: FileMetadata) -> Path:
        """Get the local target directory for cloud sensor data."""
        return Path(self.config.temp_dir) / 'templogs/cloudsensor' / metadata.site
    ...
    def process_cloud_file(self, cloud_file_key: str):
        ...
        local_target_dir = Path(self.config.temp_dir) / 'templogs/cloudsensor' / site
        ...
        if self.cloud_storage.download_file(cloud_file_key, str(local_file)):
            ...
            cloud_target_key = f"{self.config.aws_data_prefix}templogs/cloudsensor/{site}/{filename}"
