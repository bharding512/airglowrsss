...
NUM_PROCESSORS = 4
logging.basicConfig(filename='../notebooks/NikitaExample.log', encoding='utf-8',
    format='%(asctime)s %(levelname)-8s %(message)s',
    level=logging.INFO,
    datefmt='%Y-%m-%d %H:%M:%S')
logging.debug("Date: 04/11/23")
...
def spatial_cnv(args):
    ...
    imgi, gl, gk = args
#     logging.debug("Spatial_cnv IMGI: %s" , imgi)
#     logging.debug("Spatial_cnv gl: %s" , gl)
#     logging.debug("Spatial_cnv gk: %s" , gk)
    image = data_frames[imgi]
    glk   = g_kernels[gl][gk].real
    t2 = time.perf_counter()
#     print("Loading in images: ", t2 - t1)
    resize_size = 512
    
    from skimage import io, transform
    import scipy
#     image_resized = transform.resize(image, (resize_size, resize_size))
    
    
#     image_resized = scipy.ndimage.zoom(image, .25, order=3)

#     glk_resized = np.resize(glk,(resize_size,resize_size))
    
#     logging.debug("Spatial_cnv Image: %s" , image)
#     logging.debug("Spatial_cnv IMGI: %s" , imgi)
#     logging.debug("Spatial_cnv glk: %s" , glk)
#     logging.debug("In Spatial_cnv:  Image shape is %s and glk shape is %s)" , np.shape(image), np.shape(glk))
    
#     print("GLK AND IMAGE SIZE", np.shape(glk), np.shape(image_resized))
    t1 = time.perf_counter()
    cnv    = signal.fftconvolve(image, glk, mode='same')
#     print("This is the convolution: ", cnv)
    t2 = time.perf_counter()
    print("Convolving Time with image size  ", t2 - t1, resize_size)
#     r("Spatial_cnv signalfftConvolve time is %f", (t2-t1))
    
    
    
#     glk1 = np.array(glk).flatten()
#     image1 = np.array(image).flatten()
   
#     cnv1 = np.convolve(image1, glk1, mode='full')
               
    
#     print("Spatial_cnv npconvolve time is %f", (t4-t3))
    
    
#     t5 = time.perf_counter()
#     cnv2 = signal.convolve(image, glk, mode='full')
#     t6 = time.perf_counter()
#     logging.debug("Spatial_cnv signal.convolve time is %f", (t6-t5))
    N      = cnv.shape[0]
    M      = cnv.shape[1]
    t1 = time.perf_counter()
    abs_cnv = np.abs(cnv)
    e1 = np.sum(abs(cnv)**2)/(M*N)
    t2 = time.perf_counter()
#     print("Numpy way ", t2 - t1)
    ...
    t2 = time.perf_counter()
    print("end Convolving ", t2 - t1)

    print("energy is ", energy)

    return energy

def Energies_Direct_PLL_Modified(i):

    global data_frames
    global g_kernels
    
    
    lenGKernel = len(g_kernels)
    widthGKernel = len(g_kernels[0][:])

    energies  = np.array((lenGKernel, widthGKernel))
    args = np.arange(0, len(g_kernels))
    args.tile(args, [len(g_kernels[0][:]), 1])
...
#    pool.join()
    ...
    for a in args:
        energies_LIST.append(spatial_cnv(a))
#     logging.debug("Energies_list shape: %s", np.shape(energies_LIST))
#     logging.debug("Energies_list values: %s", energies_LIST)
    t2 = time.perf_counter()
    print("energies _list time: ", t2 - t1)
    p = 0
    ...
    for l in range(0, len(g_kernels)):
        for k in range(0, len(g_kernels[l][:])):
            ...
            p = p + 1
    t2 = time.perf_counter()
    print("energies mapping: ", t2-t1)
    logging.debug("----------------------------------")

    return energies, args
