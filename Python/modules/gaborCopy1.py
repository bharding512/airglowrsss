...
def Create_FilterBank_Gabor_test2(n_stdsarg, carriers, orientations, Bfarg, Bthetaarg, DCarg = False):
...
#    print("Constructing filter bank...", end = '')
    ...
    sigma_x_max   = (max_lam)*sqrt(2*log(2))/(2*pi*tanh(Bf/2.*log(2)))
    sigma_y_max   = (1/carriers)*sqrt(2*log(2))/(2*pi)*1./tan(Btheta/2.)
    
    
    xa1 = np.abs(n_stds * sigma_x_max * np.cos(orientations))
    xa2 = np.abs(n_stds * np.multiply(sigma_y_max , np.sin(orientations)))
    xa  = np.concatenate([xa1, xa2, [1]])


    ya1 = np.abs(n_stds * np.multiply(sigma_y_max , np.cos(orientations)))
    ya2 = np.abs(n_stds * sigma_x_max * np.sin(orientations))
    ya  = np.concatenate([ya1, ya2, [1]])
