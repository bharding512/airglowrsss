...
m = Basemap(llcrnrlon=-45.,llcrnrlat=-15.,urcrnrlon=-30.,urcrnrlat=0.,
...
m.drawparallels(np.arange(-15,0,5),labels=[1,1,0,0])
m.drawmeridians(np.arange(-45,-30,5),labels=[0,0,0,1])
...
for s in sites:
    ...
    dns, sky_temp, amb_temp = bs.ReadRawTempLog('/home/master/RENOIR/Control/'+s+'Cloud.txt', sites[s]['tz'])
    ...
    f = glob.glob('/home/master/RENOIR/Control/'+ s + 'Control.txt')
...
#    if abs(utc.localize(a) - local.localize(datetime.datetime.now())) > datetime.timedelta(minutes = 15):
    ...
    plt.title(local.localize(datetime.datetime.now()).astimezone(utc).strftime('RENOIR %Y-%m-%d %H:%M:%S UT'))

    plt.savefig('/usr/share/drupal7/images/RENOIRmap_RT.png')
